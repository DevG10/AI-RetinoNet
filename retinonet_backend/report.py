...
def generate_pdf_report(predictions, image_path):
    """Generate a visually enhanced PDF report with professional styling."""
    buffer = io.BytesIO()
    ...
    # Header with Logo
    try:
        logo_path = os.getenv("assets/logo.png")
        ...
        report_content.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor('#1E40AF')))
    except Exception as e:
        report_content.append(Paragraph("<b>RetinoNet Diagnostics</b>", title_style))
    ...
    prediction_data = []
    confidences = [float(value.strip('%')) / 100 for value in predictions.values()]
    sorted_indices = np.argsort(confidences)[::-1]

    for idx in sorted_indices:
        class_name = list(predictions.keys())[idx]
        ...
        prediction_data.append([
            ...
            Paragraph(f"{confidence:.2%}", ParagraphStyle('Confidence', textColor=color))
        ])
    ...
    report_content.append(Paragraph(recommendation, styles['BodyText']))
...
def get_recommendation(predictions):
    ...
    try:
        ...
        if isinstance(predictions, np.ndarray):
            logging.warning("Predictions received as numpy array; converting to dictionary.")
            predictions = {CLASS_NAMES[i]: f"{predictions[i] * 100:.2f}%" for i in range(len(predictions))}

        # Convert percentage strings to float values
        float_predictions = {k: float(v.strip('%')) / 100 for k, v in predictions.items()}

        # Find the class with the highest probability
        top_prediction = max(float_predictions, key=float_predictions.get)

        return recommendations.get(top_prediction, "No recommendation available.")
